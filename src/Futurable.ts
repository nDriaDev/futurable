...
export class Futurable<T> extends Promise<T> {
	...
	*/
	delay<TResult1 = T, TResult2 = never>(cb: (val: T) => TResult1 | PromiseLike<TResult1> | FuturableLike<TResult1>, timer: number): Futurable<TResult1 | TResult2> {
		return this.then(
			val => {
				return new Futurable<TResult1>((resolve) => {
					this.idsTimeout.push(
						setTimeout(() => resolve(cb(val)), timer)
					);
				}, this.internalSignal);
			}
		);
	...
	*/
	onCancel<TResult1 = void, TResult2 = never>(cb: () => void): Futurable<TResult1 | TResult2> {
		const cancelable = new Futurable<TResult1 | TResult2>((resolve, reject, utils) => {
			utils.onCancel(cb);
			this.then(
		...
		cancelable.controller = this.controller;
		return cancelable;
	}
	...
	*/
	futurizable<TResult1 = T, TResult2 = never>(promise: Promise<TResult1> | ((val?: T) => Promise<TResult1>)): Futurable<TResult1 | TResult2> {
		return this.then(val => {
			const resolvedPromise = typeof promise === "function" ? promise(val) : promise;
			return new Futurable<TResult1>((resolve, reject) => {
				resolvedPromise
					.then(resolve)
					.catch(reject);
			}, this.internalSignal);
